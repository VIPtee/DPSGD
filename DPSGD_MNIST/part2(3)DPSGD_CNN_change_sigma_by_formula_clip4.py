...
def main(_):
    ...
    j=0
    workbook = xlsxwriter.Workbook('/home/yxx/PythonProjects/PrivateDeepLearning-master/big_paper/part2(3)DPSGD_CNN_change_sigma_by_formula_clip4.xlsx')
    ...
    for i in range(1,num_steps+1):
        ...
        if i % 6000 == 0:
            ...
            test_accuracy=accuracy.eval(feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
            train_accuracy = accuracy.eval(feed_dict={x: mnist.train.images, y_: mnist.train.labels, keep_prob: 1.0})
            print("step \t %d \t test accuracy \t %g \t train accuracy \t %g\t now_learning_rate=%g\t now_sigma=%g" % (i/60, test_accuracy,train_accuracy,now_learning_rate_by_epoch,now_sigma_by_epoch));
            j=j+1
