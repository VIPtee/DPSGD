...
mnist_private = input_data.read_data_sets("MNIST_private_55200/", one_hot=True);
mnist_public = input_data.read_data_sets("MNIST_private_4800/", one_hot=True);
...
constant_value_4 = tf.constant(4.0)

def compute_new_clip_bound_public(y_, y_conv, public_batch_size, W_fc1, b_fc1, W_fc2, b_fc2):
    sum_now_gl2=None
    ...
    sum_now_gl2_list = []
    for batch_i in range(public_batch_size):
        ...
        sum_now_gl2_list.append(now_gl2)
        # if(batch_i==0):
        #     sum_g_W_fc1 = tf.abs(g_W_fc1)
        #     sum_g_b_fc1 = tf.abs(g_b_fc1)
        #     sum_g_W_fc2 = tf.abs(g_W_fc2)
        #     sum_g_b_fc2 = tf.abs(g_b_fc2)
        # else:
        #     sum_g_W_fc1 += tf.abs(g_W_fc1)
        #     sum_g_b_fc1 += tf.abs(g_b_fc1)
        #     sum_g_W_fc2 += tf.abs(g_W_fc2)
        ...
        if (batch_i == 0):
            ...
            sum_g_W_fc2 += g_W_fc2
            sum_g_b_fc2 += g_b_fc2
    sum_g_W_fc1 /= public_batch_size
    sum_g_b_fc1 /= public_batch_size
    sum_g_W_fc2 /= public_batch_size
    sum_g_b_fc2 /= public_batch_size
    # now_gl2 = tf.sqrt(2.0 * (tf.nn.l2_loss(sum_g_W_fc2)))
    now_gl2 = tf.sqrt(2.0 * (tf.nn.l2_loss(sum_g_W_fc1) + tf.nn.l2_loss(sum_g_b_fc1) + tf.nn.l2_loss(sum_g_W_fc2) + tf.nn.l2_loss(sum_g_b_fc2)))

        # sum_now_gl2_list.append(now_gl2)
        # g_W_fc1 = g_W_fc1 / tf.maximum(constant_value_1, now_gl2 / 4)
        # g_b_fc1 = g_b_fc1 / tf.maximum(constant_value_1, now_gl2 / 4)
        # g_W_fc2 = g_W_fc2 / tf.maximum(constant_value_1, now_gl2 / 4)
        # g_b_fc2 = g_b_fc2 / tf.maximum(constant_value_1, now_gl2 / 4)
        # now_gl2 = tf.sqrt(2.0 * (tf.nn.l2_loss(g_W_fc1) + tf.nn.l2_loss(g_b_fc1) + tf.nn.l2_loss(g_W_fc2) + tf.nn.l2_loss(g_b_fc2)))
        # if(batch_i==0):
        #     sum_now_gl2 = now_gl2
        # else:
        #     sum_now_gl2 = sum_now_gl2 + now_gl2
    # sum_now_gl2 = sum_now_gl2/batch_size
    # return sum_now_gl2,sum_now_gl2_list
    return now_gl2,sum_now_gl2_list
...
def main(_):
    ...
    public_batch_size=600
    sample_rate = 600 / 55200  # 'sample rate q = L / N'
    ...
    # 连一个全连接层
    W_fc1 = weight_variable([784, 1000]);#2层 构建新的卷积核，用来进行全连接层运算，通过这个卷积核，将最后一个池化层的输出数据转化为一维的向量1*1024。
    ...
    b_fc1 = bias_variable([1000]);  # 构建1*1024的偏置；
    ...
    average_l2norm, average_l2norm_list = compute_new_clip_bound_public(y_, y_conv, public_batch_size, W_fc1, b_fc1, W_fc2,b_fc2)
    ...
    j=0
    workbook = xlsxwriter.Workbook('/home/yxx/PythonProjects/PrivateDeepLearning-master/big_paper/[mnist](3)(2)DPSGD_CNN_change_clipbound_with_pub_dataset_sigma4.xlsx')
    worksheet = workbook.add_worksheet()
    worksheet.write(0, 0, 'epoch')
    worksheet.write(0, 1, 'train_accuracy')
    worksheet.write(0, 2, 'test_accuracy')
    worksheet.write(0, 3, 'clip_bound')

    for i in range(1,num_steps+1):

        if(i<=10*92):
            now_learning_rate_by_epoch=0.1-0.0048*i/92
        else:
            now_learning_rate_by_epoch = 0.052

        batch_public = mnist_public.train.next_batch(public_batch_size);
        batch_private = mnist_private.train.next_batch(batch_size);
        # if(i%60==1):
        #     batch_public = mnist_public.train.next_batch(public_batch_size); #这个batch_size指的是每次从公共数据集中取10个样本
        now_average_l2norm,now_average_l2norm_list = sess.run([average_l2norm,average_l2norm_list], feed_dict={x: batch_public[0], y_: batch_public[1], keep_prob: 1.0})
        # sum_clip_bound=0
        # for j in range(batch_size):  # 输出前10个
        #     sum_clip_bound=sum_clip_bound+now_average_l2norm_list[j]
        now_clip_bound = now_average_l2norm
        # now_average_l2norm_list.sort()
        # now_clip_bound = now_average_l2norm
        now_sensitivity = now_clip_bound
        # file_handle = open('/home/min/文档/'+str(i)+'.txt', mode='w')
        # for j in range(batch_size):#输出前10个
        #     # print("%g" % (now_average_l2norm_list[j]))
        #     file_handle.write(str(j)+":"+str(now_average_l2norm_list[j])+"\n")
        # file_handle.write('=======================================================================')
        # file_handle.write(str(now_average_l2norm_list[10]) + "\n")
        # file_handle.close()

        now_gw_Wf1,now_gbf1,now_gw_Wf2,now_gbf2 = sess.run([gw_Wf1,gbf1,gw_Wf2,gbf2], feed_dict={x: batch_private[0], y_: batch_private[1], keep_prob: 1.0, clip_bound:now_clip_bound, sensitivity:now_sensitivity})
        ...
        train_step.run(feed_dict={  now_learning_rate:now_learning_rate_by_epoch,update_gw_Wf1: now_gw_Wf1,update_gbf1: now_gbf1,  update_gw_Wf2: now_gw_Wf2, update_gbf2: now_gbf2, clip_bound:now_clip_bound, sensitivity:now_sensitivity});
        ...
        if i % 92 == 0:
            ...
            test_accuracy=accuracy.eval(feed_dict={x: mnist_private.test.images, y_: mnist_private.test.labels, keep_prob: 1.0})
            train_accuracy = accuracy.eval(feed_dict={x: mnist_private.train.images, y_: mnist_private.train.labels, keep_prob: 1.0})
            print("epoch \t %d \t test accuracy \t %g \t train accuracy \t %g \t clip_bound=%g \t sensitivity=%g" % (i/92, test_accuracy,train_accuracy,now_clip_bound,now_sensitivity));
            j=j+1
            worksheet.write(j, 0, j)
            worksheet.write(j, 1, train_accuracy)
            worksheet.write(j, 2, test_accuracy)
            worksheet.write(j, 3, now_clip_bound)
            epsilon = comp_eps(32, sample_rate, sigma, i, delta)  # 使用给定sigma的abadi代码计算epsilon
            if(epsilon>2):
                break;
    ...
    print("test accuracy %g" % accuracy.eval(feed_dict={x: mnist_private.test.images, y_: mnist_private.test.labels, keep_prob: 1.0}));
    print("train accuracy %g" % accuracy.eval(feed_dict={x: mnist_private.train.images, y_: mnist_private.train.labels, keep_prob: 1.0}));
