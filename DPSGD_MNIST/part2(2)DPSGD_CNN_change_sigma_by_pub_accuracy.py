...
# change sigma, 计算epsilon
log_moments = []
isornot_init_flag = 0
def comp_eps(lmbda, q, sigma, T, delta):
    global isornot_init_flag
    lmbds = range(1, lmbda + 1)
    if (isornot_init_flag == 0):
        for lmbd in lmbds:
            log_moment = compute_log_moment(q, sigma, 1, lmbd)
            log_moments.append([lmbd, log_moment])
        isornot_init_flag = 1
    else:
        for lmbd in lmbds:
            log_moment = compute_log_moment(q, sigma, 1, lmbd)
            log_moments[lmbd - 1][1] = log_moments[lmbd - 1][1] + log_moment
        # if(T==100):
        #   log_moment = compute_log_moment(q, sigma, 5400, lmbd)
        # log_moments.append([lmbd, log_moment])
...
def main(_):
    ...
    now_sigma = tf.placeholder(tf.float32, None)
    ...
    gw_Wf1,gbf1,gw_Wf2,gbf2 = compute_new_gradients(y_, y_conv, batch_size, W_fc1, b_fc1, W_fc2, b_fc2, clip_bound, now_sigma, sensitivity)
    ...
    train_step = opt.apply_gradients([(update_gw_Wf1+tf.random_normal(shape=tf.shape(update_gw_Wf1), mean=0.0, stddev=now_sigma * sensitivity,dtype=tf.float32)/lot_size, W_fc1),
                                      (update_gbf1+tf.random_normal(shape=tf.shape(update_gbf1), mean=0.0, stddev=now_sigma * sensitivity,dtype=tf.float32)/lot_size, b_fc1),
                                      (update_gw_Wf2+tf.random_normal(shape=tf.shape(update_gw_Wf2), mean=0.0, stddev=now_sigma * sensitivity,dtype=tf.float32)/lot_size, W_fc2),
                                      (update_gbf2+tf.random_normal(shape=tf.shape(update_gbf2), mean=0.0, stddev=now_sigma * sensitivity,dtype=tf.float32)/lot_size, b_fc2)]);
    ...
    j=0
    workbook = xlsxwriter.Workbook('/home/yxx/PythonProjects/PrivateDeepLearning-master/big_paper/part2(2)DPSGD_CNN_change_sigma_by_pub_accuracy.xlsx')
    worksheet = workbook.add_worksheet()
    worksheet.write(0, 0, 'epoch')
    worksheet.write(0, 1, 'train_accuracy')
    worksheet.write(0, 2, 'test_accuracy')
    worksheet.write(0, 3, 'now_sigma')
    flag=0
    public_accuracy=[]
    before_public_accuracy=0
    now_sigma_by_epoch=10
    for i in range(1,num_steps+1):
        ...
        #     update_flag = 0
        if(i<=10*5520):
            now_learning_rate_by_epoch=0.1-0.0048*i/5520
        else:
            now_learning_rate_by_epoch = 0.052



        batch = mnist_private.train.next_batch(batch_size);
        ...
        if(i%60==1):
            ...
            if (i % 60 == 0):
                ...
                train_step.run(feed_dict={ now_sigma:now_sigma_by_epoch,now_learning_rate:now_learning_rate_by_epoch, update_gw_Wf1: now_gw_Wf1,update_gbf1: now_gbf1,  update_gw_Wf2: now_gw_Wf2, update_gbf2: now_gbf2});
                ...
                epsilon = comp_eps(32, sample_rate, now_sigma_by_epoch, i/60, delta)  # 使用给定sigma的abadi代码计算epsilon
                ...
                if(epsilon>0.5):
                    ...
                    print("epoch \t %d \t test accuracy \t %g \t train accuracy \t %g\t now_learning_rate=%g" % (i / 5520, test_accuracy, train_accuracy, now_learning_rate_by_epoch));
                    if(i<=5520):
        ...
        if i % 5520 == 0:
            ...
            # if (i <= 100 * 6000):
            #     now_sigma_by_epoch = 6 - 0.04 * i / 6000
            #     # now_sigma_by_epoch = 8
            # else:
            #     now_sigma_by_epoch = 2
            public_train_accuracy = accuracy.eval(feed_dict={x: mnist_public.train.images, y_: mnist_public.train.labels, keep_prob: 1.0})
            # public_accuracy.append(public_train_accuracy)
            # if(public_train_accuracy-before_public_accuracy<=0.01):
            #     now_sigma_by_epoch = now_sigma_by_epoch * 0.7
            # before_public_accuracy=public_train_accuracy
            if(i / 5520<=5):
                now_sigma_by_epoch=now_sigma_by_epoch
                public_train_accuracy = accuracy.eval(feed_dict={x: mnist_public.train.images, y_: mnist_public.train.labels, keep_prob: 1.0})
                public_accuracy.append(public_train_accuracy)
                print(public_train_accuracy)
            else:
                public_train_accuracy = accuracy.eval(feed_dict={x: mnist_public.train.images, y_: mnist_public.train.labels, keep_prob: 1.0})
                public_accuracy.append(public_train_accuracy)
                # print(public_train_accuracy)
                ...
                acc_now=0
                for k in range(1,6):
                    acc_before=acc_before+public_accuracy[math.floor(i/5520)-1-k]
                    acc_now=acc_now+public_accuracy[math.floor(i/5520)-k]
                    # print(public_train_accuracy)
                # print("1:%g"%acc_before)
                # print("2:%g" % acc_now)
                # print("3:%g" % (acc_before/5.0))
                # print("4:%g" % (acc_now/5.0))
                if(acc_now/5.0-acc_before/5.0<=0.01):
                    now_sigma_by_epoch = now_sigma_by_epoch*0.7
            test_accuracy=accuracy.eval(feed_dict={x: mnist_private.test.images, y_: mnist_private.test.labels, keep_prob: 1.0})
            train_accuracy = accuracy.eval(feed_dict={x: mnist_private.train.images, y_: mnist_private.train.labels, keep_prob: 1.0})
            print("epoch \t %d \t test accuracy \t %g \t train accuracy \t %g\t now_learning_rate=%g\t now_sigma=%g" % (math.floor(i/5520), test_accuracy,train_accuracy,now_learning_rate_by_epoch,now_sigma_by_epoch));
            j=j+1
            worksheet.write(j, 0, j)
            worksheet.write(j, 1, train_accuracy)
            worksheet.write(j, 2, test_accuracy)
            worksheet.write(j, 3, now_sigma_by_epoch)
            print("epsilon: {}".format(epsilon))
            #
            ...
            if(i==5520):
                ...
                max_test_accuracy = test_accuracy
                max_test_accuracy_epsilon =epsilon
                max_test_accuracy_epoch=math.floor(i/5520)
