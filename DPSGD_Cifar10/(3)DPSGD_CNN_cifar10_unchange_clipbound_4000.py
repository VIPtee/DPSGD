...
def compute_new_gradients(y_, y_conv, batch_size,W_fc1, b_fc1, W_fc2, b_fc2, W_fc3, b_fc3, clip_bound, sigma, sensitivity):
  # sum_g_W_1 = None
  # sum_g_b_1 = None
  # sum_g_W_2 = None
  # sum_g_b_2 = None
  ...
  for batch_i in range(batch_size):
      ...
      cross_entropy= loss(y_conv[batch_i], y_[batch_i])  # 定义loss
      ...
      now_gl2 = tf.sqrt(2.0 * (tf.nn.l2_loss(g_W_fc1)+tf.nn.l2_loss(g_b_fc1)
      ...
      g_b_fc2 = g_b_fc2 / tf.maximum(constant_value_1, now_gl2 / clip_bound) #4
      g_W_fc3 = g_W_fc3 / tf.maximum(constant_value_1, now_gl2 / clip_bound)   #1        1        4        4       0.5          1         1            0.1         1
      g_b_fc3 = g_b_fc3 / tf.maximum(constant_value_1, now_gl2 / clip_bound) #4
      # g_W_fc1 = g_W_fc1 / tf.maximum(constant_value_1, now_gl2 / clip_bound)   #1 91.26% 1 91.33% 4 88.5% 4 89.39% 0.5  89.91%  1 91.35%   1   91.61%  0.1  81.4%  1   91.61%
      # g_b_fc1 = g_b_fc1 / tf.maximum(constant_value_1, now_gl2 / clip_bound) #4        1        4        1       0.5          0.5       0.1            0.1      0.05
      # g_W_fc2 = g_W_fc2 / tf.maximum(constant_value_1, now_gl2 / clip_bound)   #1        1        4        4       0.5          1         1            0.1         1
      ...
      if(batch_i==batch_size-1):
        ...
        sum_g_b_fc3 += g_b_fc3#
        # sum_g_wb = sum_g_wb
        return sum_g_W_fc1,sum_g_b_fc1,sum_g_W_fc2,sum_g_b_fc2,sum_g_W_fc3,sum_g_b_fc3
      elif(batch_i==0):
        # sum_g_W_1 = g_W_1  # + tf.random_normal(shape=tf.shape(g_W_fc1), mean=0.0, stddev=sigma * sensitivity,dtype=tf.float32)/batch_size
        # sum_g_b_1 = g_b_1
        # sum_g_W_2 = g_W_2  # + tf.random_normal(shape=tf.shape(g_W_fc1), mean=0.0, stddev=sigma * sensitivity,dtype=tf.float32)/batch_size
...
def main(_):
    ...
    sample_rate = 4000 / 50000  # 'sample rate q = L / N'
    # num_steps = 160000  # 'number of steps T = E * N / L = E / q'
    num_steps = 920*5000 #epochs*train_images_number/10
    ...
    # print(y_conv)
    gw_Wf1,gbf1,gw_Wf2,gbf2,gw_Wf3,gbf3 = compute_new_gradients(y_, y_conv, batch_size, W_fc1, b_fc1, W_fc2, b_fc2, W_fc3, b_fc3,clip_bound, sigma, sensitivity)
    ...
    train_step = opt.apply_gradients([(update_gw_Wf1 + tf.random_normal(shape=tf.shape(update_gw_Wf1), mean=0.0,
                                                                        stddev=sigma * sensitivity,
                                                                        dtype=tf.float32) / lot_size, W_fc1),
                                      (update_gbf1 + tf.random_normal(shape=tf.shape(update_gbf1), mean=0.0,
                                                                      stddev=sigma * sensitivity,
                                                                      dtype=tf.float32) / lot_size, b_fc1),
                                      (update_gw_Wf2 + tf.random_normal(shape=tf.shape(update_gw_Wf2), mean=0.0,
                                                                        stddev=sigma * sensitivity,
                                                                        dtype=tf.float32) / lot_size, W_fc2),
                                      (update_gbf2 + tf.random_normal(shape=tf.shape(update_gbf2), mean=0.0,
                                                                      stddev=sigma * sensitivity,
                                                                      dtype=tf.float32) / lot_size, b_fc2),
                                      (update_gw_Wf3 + tf.random_normal(shape=tf.shape(update_gw_Wf3), mean=0.0,
                                                                        stddev=sigma * sensitivity,
                                                                        dtype=tf.float32) / lot_size, W_fc3),
                                      (update_gbf3 + tf.random_normal(shape=tf.shape(update_gbf3), mean=0.0,
                                                                      stddev=sigma * sensitivity,
                                                                      dtype=tf.float32) / lot_size, b_fc3)
                                      ]);
    #
    ...
    j=0
    workbook = xlsxwriter.Workbook('/home/yxx/PythonProjects/PrivateDeepLearning-master/big_paper/test3_cifar10_unchanged_4000.xlsx')
    ...
    for i in range(1,num_steps+1):
        ...
        if(i%400==1):
            ...
            now_gw_Wf1,now_gbf1,now_gw_Wf2,now_gbf2,now_gw_Wf3,now_gbf3 = sess.run([gw_Wf1,gbf1,gw_Wf2,gbf2,gw_Wf3,gbf3], feed_dict={x: image_batch, y_: label_batch, keep_prob: 0.5, clip_bound:now_clip_bound, sensitivity:now_sensitivity})
            #print("aaa")
            ...
            now_gw_Wf1_i, now_gbf1_i, now_gw_Wf2_i, now_gbf2_i, now_gw_Wf3_i, now_gbf3_i = sess.run([gw_Wf1,gbf1,gw_Wf2,gbf2,gw_Wf3,gbf3],feed_dict={x: image_batch, y_: label_batch, keep_prob: 0.5, clip_bound:now_clip_bound, sensitivity:now_sensitivity})
            ...
            if (i % 400 == 0):
                ...
                train_step.run(feed_dict={ now_learning_rate:now_learning_rate_by_epoch, update_gw_Wf1: now_gw_Wf1,update_gbf1: now_gbf1,  update_gw_Wf2: now_gw_Wf2, update_gbf2: now_gbf2, update_gw_Wf3: now_gw_Wf3, update_gbf3: now_gbf3,clip_bound:now_clip_bound, sensitivity:now_sensitivity});
        ...
        if i % 5000 == 0:
            ...
            epsilon = comp_eps(32, sample_rate, sigma, i/400, delta)  # 使用给定sigma的abadi代码计算epsilon
