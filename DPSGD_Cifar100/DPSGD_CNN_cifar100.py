...
from dpSGD.dpSGD_Cifar100 import cifar10, cifar10_input,cifar100_input
...
data_dir = './cifar-100-binary'  # 数据所在路径
...
# images_train, labels_train = cifar100_input.distorted_inputs(data_dir=data_dir,
#                                                             batch_size=batch_size)
# # 裁剪图片正中间的24*24大小的区块并进行数据标准化操作
# images_test, labels_test = cifar100_input.inputs(eval_data=True,
#                                                 data_dir=data_dir,
#                                                 batch_size=batch_size)


# dataset_dir_cifar10 = '../CIFAR10_dataset'
# dataset_dir_cifar100 = './dpSGD_Cifar100'

num_examples_per_epoch_for_train = cifar100_input.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN#50000
num_examples_per_epoch_for_eval = cifar100_input.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL#10000
image_size = cifar100_input.IMAGE_SIZE
image_channel = cifar100_input.IMAGE_DEPTH

# cifar10_data_url = cifar100_input.CIFAR10_DATA_URL
cifar100_data_url = cifar100_input.CIFAR100_DATA_URL
...
#     n_classes = cifar100_input.NUM_CLASSES_CIFAR10
#     dataset_dir = dataset_dir_cifar10
#
# if cifar10or20or100 == 20:
#     n_classes = cifar100_input.NUM_CLASSES_CIFAR20
#     dataset_dir = dataset_dir_cifar100
#
# if cifar10or20or100 == 100:
#     n_classes = cifar100_input.NUM_CLASSES_CIFAR100
#     dataset_dir = dataset_dir_cifar100
...
def get_distorted_train_batch(cifar10or20or100,batch_size):
    ...
    images, labels = cifar100_input.distorted_inputs(cifar10or20or100=cifar10or20or100,data_dir=data_dir,batch_size=batch_size)
    return images,labels

def get_undistorted_eval_batch(cifar10or20or100,eval_data, batch_size):
    ...
    #     data_dir = os.path.join(data_dir, 'cifar-100-binary')
    data_dir = './cifar-100-binary'
    images, labels = cifar100_input.inputs(cifar10or20or100=cifar10or20or100,eval_data=eval_data,data_dir=data_dir,batch_size=batch_size)
    return images,labels


images_train,labels_train = get_distorted_train_batch(cifar10or20or100=cifar10or20or100,batch_size=batch_size)


images_test, labels_test = get_undistorted_eval_batch(cifar10or20or100=cifar10or20or100,eval_data=True, batch_size=batch_size)
...
def compute_new_gradients(y_, y_conv, batch_size, W1,b1,W2,b2,W_fc1, b_fc1, W_fc2, b_fc2, W_fc3, b_fc3, clip_bound, sigma, sensitivity):
  ...
  for batch_i in range(batch_size):
      ...
      if(batch_i==batch_size-1):
        ...
        sum_g_b_fc3 += g_b_fc3#
        # sum_g_wb = sum_g_wb
        return sum_g_W_1,sum_g_b_1,sum_g_W_2,sum_g_b_2,sum_g_W_fc1,sum_g_b_fc1,sum_g_W_fc2,sum_g_b_fc2,sum_g_W_fc3,sum_g_b_fc3
      elif(batch_i==0):
        sum_g_W_1 = g_W_1  # + tf.random_normal(shape=tf.shape(g_W_fc1), mean=0.0, stddev=sigma * sensitivity,dtype=tf.float32)/batch_size
        sum_g_b_1 = g_b_1
        sum_g_W_2 = g_W_2  # + tf.random_normal(shape=tf.shape(g_W_fc1), mean=0.0, stddev=sigma * sensitivity,dtype=tf.float32)/batch_size
...
def main(_):
    ...
    # 形成防止过拟合之后的矩阵。
    W_fc3 = variable_with_weight_loss([384, 100], stddev=1 / 384.0, wl=0.0)
    ...
    # print(y_conv)
    gw_W1,gb1,gw_W2,gb2,gw_Wf1,gbf1,gw_Wf2,gbf2,gw_Wf3,gbf3 = compute_new_gradients(y_, y_conv, batch_size, W_conv1, b_conv1, W_conv2, b_conv2, W_fc1, b_fc1, W_fc2, b_fc2, W_fc3, b_fc3,clip_bound, sigma, sensitivity)
    ...
    train_step = opt.apply_gradients([(update_gw_W1, W_conv1),
                                      (update_gb1, b_conv1),
                                      (update_gw_W2, W_conv2),
                                      (update_gb2, b_conv2),
                                      (update_gw_Wf1, W_fc1),
                                      (update_gbf1, b_fc1),
                                      (update_gw_Wf2, W_fc2),
    ...
    for i in range(1,num_steps+1):
        ...
        if(i%200==1):
            ...
            now_gw_W1,now_gb1,now_gw_W2,now_gb2,now_gw_Wf1,now_gbf1,now_gw_Wf2,now_gbf2,now_gw_Wf3,now_gbf3 = sess.run([gw_W1,gb1,gw_W2,gb2,gw_Wf1,gbf1,gw_Wf2,gbf2,gw_Wf3,gbf3], feed_dict={x: image_batch, y_: label_batch, keep_prob: 0.5, clip_bound:now_clip_bound, sensitivity:now_sensitivity})
            #print("aaa")
        ...
        if i % 5000 == 0:
            ...
            print("epoch \t %d \t test accuracy \t %g \t clip_bound=%g \t sensitivity=%g\tnow_learning_rate=%g" % (i/5000, test_accuracy,now_clip_bound,now_sensitivity,now_learning_rate_by_epoch));
